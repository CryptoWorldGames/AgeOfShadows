...
export function createStone(scene, position = { x: 0, y: 0, z: 0 }) {
  ...
  main.position.y = 0.5; main.scale.set(1, 0.75, 0.9);
  ...
  mid.position.set(0.7, 0.3, 0.2); mid.scale.set(0.9, 0.7, 0.8);
  mid.rotation.set(Math.random(), Math.random(), Math.random());
  mid.castShadow = true; mid.receiveShadow = true; group.add(mid);
  ...
  small.castShadow = true; group.add(small);
  ...
  tiny.castShadow = true; group.add(tiny);
  ...
  for (let i = 0; i < 5; i++) {
    const chunk = new THREE.Mesh(new THREE.DodecahedronGeometry(0.12 + Math.random() * 0.1, 0), pileMat);
    chunk.position.set((Math.random() - 0.5) * 0.6, 0.1, (Math.random() - 0.5) * 0.6);
    chunk.rotation.set(Math.random(), Math.random(), Math.random());
    chunk.castShadow = true; pile.add(chunk);
  }
  ...
  let sinkT = 0;
  let hitFlashT = 0;
  const allRocks = [main, mid, small, tiny];

  function takeDamage(d) {
    if (state !== 'standing') return;
    hp -= d;
    hitFlashT = 0.12;
    allRocks.forEach((r) => { r.material = r.material.clone(); r.material.emissive.setHex(0x333333); });
  ...
  function reset() {
    ...
    group.position.y = 0; group.visible = true; group.rotation.z = 0;
