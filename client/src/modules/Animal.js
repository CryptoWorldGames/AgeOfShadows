...
export function createChicken(scene, position = { x: 0, y: 0, z: 0 }) {
  ...
  function setWalking(w) {
    ...
    walkAction.paused = !w;
    if (w && scaredAction) scaredAction.stop();
  }
  function setScared() {
...
export function createDeer(scene, position = { x: 0, y: 0, z: 0 }) {
  ...
  getDeerGLTF().then((gltf) => {
    ...
    if (clips[0]) {
      // Use AnimationClip.parse/toJSON to fully isolate each deer's animation
      const clip = THREE.AnimationClip.parse(THREE.AnimationClip.toJSON(clips[0]));
  ...
  function update(dt, world) {
    ...
    if (idleAction) idleAction.timeScale = isRunning ? 2.5 : 1.0;
