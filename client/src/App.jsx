    ...
    if (mode === 'login') {
      ...
      try {
        const response = await fetch('/api/register', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: email.trim(), displayName: displayName.trim(), password, recaptchaToken })
        });
        const data = await response.json();
        if (!response.ok) {
  ...
  return (
    <div style={{
      ...
      <div style={{background:'rgba(0,0,0,0.7)',border:'1px solid rgba(200,168,75,0.4)',borderRadius:'16px',padding:'40px 48px',width:'360px',zIndex:1,backdropFilter:'blur(10px)',boxShadow:'0 0 60px rgba(200,168,75,0.1)'}}>
        ...
        {mode==='register'&&(<div style={{marginBottom:'20px',display:'flex',justifyContent:'center'}}><div className="g-recaptcha" data-sitekey="6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI" style={{transform:'scale(0.9)',transformOrigin:'0 0'}} data-callback="window.grecaptchaCallback"></div></div>)}
        ...
        <div style={{marginTop:'20px',textAlign:'center',fontSize:'11px',color:'rgba(255,255,255,0.3)',lineHeight:'1.6'}}>Free to play · No download required<br/>MULTIPLAYER STRATEGY<br/><span style={{fontSize:'10px',opacity:0.7}}>v1.0</span></div>
      </div>
